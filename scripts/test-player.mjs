...
r=await handlePlayerAPI(req('/api/media?url='+encodeURIComponent('https://cdn.example.com/range.mp4'),{headers:{range:'bytes=0-3'}}));assert.equal(r.status,206);assert.equal(r.headers.get('content-range'),'bytes 0-3/20');assert.equal(await r.text(),'0123');assert.equal(new Headers(remoteCalls.find(c=>c.url.endsWith('range.mp4')).options.headers).get('range'),'bytes=0-3');checks++;console.log('PASS MP4 Range 流式透传');
