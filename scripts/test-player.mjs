...
globalThis.fetch=async(input,options={})=>{
  ...
  if(url.endsWith('/redirect'))return new Response(null,{status:302,headers:{location:'http://127.0.0.1/secret'}});
  if(url.endsWith('/range.mp4'))return new Response('0123',{status:206,headers:{'content-type':'video/mp4','content-range':'bytes 0-3/20','accept-ranges':'bytes'}});
  if(url.endsWith('/error'))return new Response('not found',{status:404});
  if(url.endsWith('/page'))return new Response('<html>login</html>',{headers:{'content-type':'text/html'}});
...
r=await handlePlayerAPI(req('/api/media?url='+encodeURIComponent('https://cdn.example.com/range.mp4'),{headers:{range:'bytes=0-3'}}));assert.equal(r.status,206);assert.equal(r.headers.get('content-range'),'bytes 0-3/20');assert.equal(await r.text(),'0123');const rangeCall=remoteCalls.find(c=>c.url.endsWith('range.mp4'));assert.equal(new Headers(rangeCall.options.headers).get('range'),'bytes=0-3');assert.deepEqual(rangeCall.options.cf,{cacheTtlByStatus:{'200-599':-1}});checks++;console.log('PASS MP4 Range 流式透传');
