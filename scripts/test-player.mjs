...
globalThis.fetch=async(input,options={})=>{
  ...
  if(new URL(url).hostname==='yd-hls.bnfuiu.cn')throw new Error('Huangguo playlists must not be fetched from the Worker: '+url);
  if(new URL(url).pathname.includes('/videos5/'))return new Response('#EXTM3U\n#EXTINF:4,\nhttps://tp.example.com/videos5/other/segment.ts\n',{headers:{'content-type':'text/plain'}});
...
remoteCalls=[];r=await handlePlayerAPI(req('/api/media?url='+encodeURIComponent('https://yd-hls.bnfuiu.cn/videos5/request/request.m3u8?auth_key=test&v=3')));assert.equal(r.status,302);assert.equal(r.headers.get('location'),'https://yd-hls.bnfuiu.cn/videos5/request/request.m3u8?auth_key=test&v=3');assert.equal(r.headers.get('x-relay-fallback'),'direct-origin');assert.equal(remoteCalls.length,0);checks++;console.log('PASS 黄果清单不经 Worker 抓取，直接回源');
r=await handlePlayerAPI(req('/api/media?url='+encodeURIComponent('https://mirror.example.com/videos5/request/request.m3u8?auth_key=test')));assert.equal(r.status,302);assert.equal(r.headers.get('location'),'https://mirror.example.com/videos5/request/request.m3u8?auth_key=test');assert.equal(r.headers.get('x-relay-fallback'),'direct-origin');checks++;console.log('PASS 错位清单回退原始地址');
