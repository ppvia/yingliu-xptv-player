...
基于 Yswag/xptv-extensions 的通用网页播放器。提供视频源切换、分类、搜索、翻页、剧集分组、选集、线路选择、HLS/MP4 播放、链接播放、原始地址复制和 JSON 配置。

## 怎么用

首次打开为「播放测试」：点击公开测试片卡片，再点击「播放测试」选集，验证浏览器与网络。该条目明确是公开测试流，不代表视频源可用性。

左侧选择视频源，页面加载源脚本后显示分类和片单。选择影片、剧集即可解析并播放。中转播放用于处理 CORS、Referer 等请求头；遇到失败可切换直连、其他线路或视频源。

「添加视频源」接受指定仓库的 `js/*.js`、GitHub blob/Raw 链接或 `subs/*.json`。订阅显示条目供逐个添加，避免无意执行全部脚本。脚本始终从源仓库获取，不在服务端执行。
...
扩展配置和缓存仅存浏览器 sessionStorage；额外源列表存 localStorage，不上传配置到数据库。执行扩展会按其逻辑向源站发送请求。不要导入不信任的脚本或填写不相关的凭证。中转不提供任意协议或私网访问。媒体以流式响应转发，不整体缓存到 Worker 内存。
